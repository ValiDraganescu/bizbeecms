...
- `buildSystemPrompt`'s `tools` param is OPTIONAL by design — omitted = full prompt. Callers that can't know the scope (tests, any legacy path) must keep getting everything; don't "clean up" the fallback.
- The i18n locale rule is gated on prop-WRITING tools (create/update component/page, set_block_props), deliberately NOT on `translate` — the paragraph is entirely about locale-object props, useless to bare translate (settings context drops it on purpose).
- The base prompt's OPENING paragraph contains the words "existing components" — a test asserting the component-LIST section is absent must match the section header `This Site's existing components`, not the bare phrase.
