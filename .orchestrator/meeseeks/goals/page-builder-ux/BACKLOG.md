...
- DONE (2026-06-26): **Collapsible left rail AND right inspector.** Each side rail collapses to a thin `w-9` re-expand strip (CollapseToggle double-chevron, mirrors admin sidebar); collapsed overrides the inspector width preset. Pure helper `lib/page-builder/panel-collapse.ts` (default-expanded) + node test; persisted in `bizbee.builder.{left,right}Collapsed`. i18n `pageBuilder.panel.*` in all 3 locales.
