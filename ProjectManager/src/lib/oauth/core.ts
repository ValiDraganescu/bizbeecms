/**
 * PURE OAuth 2.1 authorization-server primitives for the PM's remote MCP server
 * (pm-mcp Slice 2). No `@/` imports, no CF bindings — Web Crypto only — so this
...
export function protectedResourceMetadata(issuer: string): Record<string, unknown> {
  ...
  return {
    ...
    resource_name: "bizbeecms ProjectManager MCP",
  };
}
...
/**
 * The public origin (issuer) to advertise: the configured `APP_ORIGIN` in
 * production; the request host in local dev. Trailing slashes stripped.
 */
export function chooseIssuer(
  ...
  return "https://manager.bizbeecms.com";
}
