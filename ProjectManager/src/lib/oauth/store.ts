/**
 * D1 persistence for the OAuth 2.1 authorization server (pm-mcp Slice 2).
...
export async function consumeCode(code: string): Promise<ConsumedCode | null> {
  ...
  const rows = await db
    ...
    .set({ usedAt: now })
    .where(and(eq(schema.oauthCodes.codeHash, codeHash), isNull(schema.oauthCodes.usedAt)))
    .returning();
  const row = rows[0];
  if (!row) return null;
...
export async function codeWasUsed(code: string): Promise<boolean> {
  ...
  const [row] = await db
    ...
    .where(eq(schema.oauthCodes.codeHash, await hashToken(code)))
    .limit(1);
...
export async function rotateTokens(
  ...
  const [row] = await db
    ...
    .where(eq(schema.oauthTokens.refreshHash, await hashToken(refreshToken)))
    .limit(1);
...
export async function findActiveAccessToken(accessToken: string): Promise<OauthToken | null> {
  ...
  const [row] = await db
    .select()
    .from(schema.oauthTokens)
    .where(eq(schema.oauthTokens.accessHash, await hashToken(accessToken)))
    .limit(1);
  if (!row || row.revokedAt != null) return null;
...
export async function revokeByToken(token: string): Promise<void> {
  ...
  await db
    ...
    .where(eq(schema.oauthTokens.accessHash, h));
  await db
    .update(schema.oauthTokens)
    .set({ revokedAt: new Date() })
    .where(eq(schema.oauthTokens.refreshHash, h));
}
...
export async function listConnections(userId: string): Promise<ConnectionItem[]> {
  ...
  const rows = await db
    .select({
      ...
      refreshExpiresAt: schema.oauthTokens.refreshExpiresAt,
...
export async function revokeConnection(userId: string, id: string): Promise<boolean> {
  ...
  const rows = await db
    .update(schema.oauthTokens)
    .set({ revokedAt: new Date() })
    .where(and(eq(schema.oauthTokens.id, id), eq(schema.oauthTokens.userId, userId)))
    .returning({ id: schema.oauthTokens.id });
