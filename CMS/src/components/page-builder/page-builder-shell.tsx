  ...
  return (
    <div className="flex h-full flex-col bg-surface">
      ...
      <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border px-4">
        {/* Left: page selector (no real pages yet — that's a later slice) */}
        <div className="flex items-center gap-2">
          <select
            disabled
            aria-label={t("pageSelector")}
            className="w-56 rounded-md border border-border bg-surface-muted px-3 py-1.5 text-sm text-foreground-muted"
          >
            <option>{t("noPageSelected")}</option>
          </select>
          <button
            type="button"
            disabled
            className="rounded-md border border-border px-2.5 py-1.5 text-sm text-foreground-muted disabled:opacity-60"
          >
            {t("newPage")}
          </button>
        </div>
