...
const MCP = "https://my-site.example.com/mcp";
...
test("metadata + challenge point at the right endpoints", () => {
  const as = authorizationServerMetadata("https://my-site.example.com/");
  assert.equal(as.issuer, "https://my-site.example.com");
  assert.equal(as.token_endpoint, "https://my-site.example.com/oauth/token");
  assert.deepEqual(as.code_challenge_methods_supported, ["S256"]);
  const pr = protectedResourceMetadata("https://my-site.example.com");
  assert.equal(pr.resource, MCP);
  assert.deepEqual(pr.authorization_servers, ["https://my-site.example.com"]);
  assert.match(wwwAuthenticate("https://my-site.example.com", "invalid_token", 'expired "now"'),
    /^Bearer resource_metadata="https:\/\/my-site\.example\.com\/\.well-known\/oauth-protected-resource\/mcp", error="invalid_token", error_description="expired 'now'"$/);
});
...
test("buildRedirect / sameResource / chooseIssuer", () => {
  ...
  assert.equal(chooseIssuer("https://my-site.example.com/", "x", null), "https://my-site.example.com");
});
