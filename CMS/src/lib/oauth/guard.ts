/**
 * Bearer-token guard for `/mcp` (cms-mcp → OAuth). MCP requests carry no cookie;
 * they present `Authorization: Bearer at_…` minted by THIS Site's own token
 * endpoint. Resolves token → grant → owning CMS `User`; fail-closed. On denial
 * the caller answers 401 with the RFC 9728 `WWW-Authenticate` challenge so the
 * client discovers our authorization server and starts the OAuth flow.
 *
 * This REPLACES the old `bzb_` API-key guard: a grant acts AS a CMS user, so
 * revoking the user (or the grant under Settings → MCP) cuts access immediately.
 */
