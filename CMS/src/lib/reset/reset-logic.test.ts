...
/**
 * auth-reset C5: BEHAVIORAL pure-logic tests for reset-token classification
 * (mirrors PM's reset-logic.test.ts). These EXECUTE the real shipped
 * `classifyReset` (which `checkReset` delegates to) and assert the actual
 * decisions: validity, the expiry boundary (just-expired vs just-valid), and
 * single-use (a used token is rejected even before expiry). No DB, no `@/`
 * alias, no frameworks — `classifyReset(row, now)` is pure.
 */
