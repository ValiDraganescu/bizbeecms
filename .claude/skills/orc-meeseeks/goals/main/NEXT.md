...
1. **DONE (this run) — per-block props editing UI.** See "DONE 2026-06-18" above. Only the per-locale prop-field expansion remains (logged as DEFERRED above).
...
**The binding loop is now CLOSED (binder + props UI both built).** A human can install a kit → fill each block's fields → see real content on the public page. Lean toward G2+ kits (#2) next, or H3 dependency handling (#3) — both build on the now-complete render/bind/edit foundation.
