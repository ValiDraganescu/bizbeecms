...
0. **Sec1 — CMS admin auth** (P0) — IF the user has chosen the arch (HITL P0). Else below. STILL OPEN — driver says don't pick (needs user arch decision). You MAY build the decision-independent seam only (a `requireAdmin(request)` stub + `/admin/*` layout guard scaffold both auth options plug into, clearly marked "no real security yet"). C2 + E1 added MORE unauthed surface (`/admin/pages` + `/api/pages`; `/admin/settings/theme` + `/api/settings/theme`) — fold them into the Sec1 gate list.
1. **E2 — site settings (brand/design/AI persona).** Smallest self-contained next win + high leverage: a `site_settings` row (`brand`/`design`/`ai_persona`) feeding the AI system prompt so generated components/pages match the Site's identity. Reuse the now-shared `upsertSetting` store helper + the C1b/E1 settings-page+REST+editor pattern (PURE normalize in `lib/...`, GET/PUT route, `/admin/settings/brand` page, EN/FI/ET namespace). Pairs with seeding the chat system prompt to mention `list_assets` + available components.
...
3. **G1 — blog component kit** (premade `{tree,script,css}` set seeded into a Site) — but H1/H2 export/import format should land first.

**Lean toward #1 (E2 brand/AI-persona)** — smallest self-contained win, follows the now-well-trodden settings pattern (E1/C1b), and directly improves AI output quality. #2 (C3 block editor) is the bigger authoring slice if you want it.
