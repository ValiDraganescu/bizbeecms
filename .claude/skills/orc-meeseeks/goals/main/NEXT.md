...
- **H1/H2 DONE (this run):** component export/import. PURE versioned format `lib/components/portable.ts` (`serializeComponent`/`parsePortableComponent`; import = trust boundary reusing `validateComponentArtifact`). REST `api/components` (GET list / `?name=` export-download / POST import). Admin `/admin/components` (export btn per component + paste/upload import). EN/FI/ET `components` namespace. CMS 182/182. Unlocks G1.
...
1. **G1 — blog starter kit.** Now UNLOCKED by H1/H2: author a curated set of blog components (PostCard / PostList / etc.) as committed portable bundles (`bizbeecms.component` v1 JSON), and a one-click "install kit" that POSTs each to `/api/components`. The import machinery + validator already exist — this is authoring the bundle CONTENT + a small install UI/seed. **LEAN HERE** — the natural next step, proves the whole H track end-to-end.
2. **H3 — dependency handling (the one real wrinkle).** A component's tree/props may reference `/media/<key>` asset URLs (per-Site R2) or other components. Export should declare deps; import should rebind/flag missing asset URLs + warn on missing nested components. Scope now that H1/H2 happy path is proven. (Could fold into G1's install flow.)
3. **C3 follow-ons (smaller):** per-block **props editing** + block-prop→component-prop binding (renderer currently IGNORES block.props — binding them is its own task; most valuable), nested `block.children` editing, or drag-and-drop reorder.
...
**Lean toward #1 (G1 blog kit)** — H1/H2 just unlocked it; it proves component portability end-to-end.
