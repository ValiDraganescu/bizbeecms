...
- **E1 DONE:** per-Site theme overrides — `theme_overrides` `site_settings` row, validated by PURE `lib/render/theme.ts` (token allowlist + safe-color grammar = security boundary for the inline `<style>`), injected after UTILITY_CSS on the public route; `/admin/settings/theme` editor + REST `api/settings/theme`; EN/FI/ET `theme` namespace. A Site re-themes color tokens without a rebuild.
- **E2 DONE (this run):** per-Site brand/design/AI-persona identity — `site_identity` `site_settings` row, PURE `lib/settings/site-settings.ts` (`normalizeSiteIdentity` + `buildSystemPrompt`); `/admin/settings/brand` editor + REST `api/settings/brand`; EN/FI/ET `brand` namespace. **AND wired into the AI**: `api/chat/route.ts` now auto-prepends a system prompt (identity + existing component names + bounded utility classes + tool guidance mentioning list_assets/components) unless the client sent its own `system` message. Prompt-wiring follow-on from E2 is DONE — no leftover.
- 5 AI tools wired: create_component, create_page, translate, list_assets (+ the chat endpoint, now with a real system prompt). Live model/D1/R2 round-trips ALL → HITL P1 (need AI binding+gateway+D1+R2).
...
0. **Sec1 — CMS admin auth** (P0) — IF the user has chosen the arch (HITL P0). Else below. STILL OPEN — driver says don't pick (needs user arch decision). You MAY build the decision-independent seam only (a `requireAdmin(request)` stub + `/admin/*` layout guard scaffold both auth options plug into, clearly marked "no real security yet"). C2/E1/E2 keep adding unauthed surface (`/admin/pages`+`/api/pages`; `/admin/settings/{theme,brand}`+`/api/settings/{theme,brand}`) — fold them all into the Sec1 gate list.
1. **C3 — section/block editing UI.** Visual compose/reorder of the page block tree — the missing half of C2 (C2 does metadata, C3 does blocks). Bigger; needs a block palette + the component registry. Pairs with C2's `/admin/pages` (add an "Edit blocks" link per page). Reuse the renderer's `planPage`/`Block` types + the component-store list. **This is now the biggest remaining authoring gap.**
2. **H1/H2 — component export/import.** Serialize a component row to portable JSON (H1) → validate+insert into another Site (H2). Trivial given no-eval data model. Unlocks **G1 blog kit** (a curated import bundle). Self-contained, offline-buildable, well-defined.
3. **F1 — PM deploy fleet view.** PM dashboard listing all Sites' deploy status + stuck flags at a glance. PM-side, reuses the stuck-detect predicates.
4. **D2 — Cloudflare Images transforms** (optional, smaller).

**Lean toward #1 (C3 block editor)** — the most valuable remaining authoring slice and the obvious partner to C2's `/admin/pages`. #2 (H1/H2 export/import) is a clean self-contained alternative if you want something smaller that unlocks G1.

### Settings pattern is now WELL-TRODDEN (C1b/E1/E2 all identical shape) — reuse for any new settings slice:
PURE normalize+logic in `lib/...` (never in `db/*` — drizzle import breaks node --test) → `db/settings-store.ts` typed accessor reusing shared `upsertSetting(key,value)` → REST `api/settings/<x>/route.ts` GET/PUT (server re-normalizes, no server actions) → `/admin/settings/<x>/page.tsx` (force-dynamic, try/catch → safe default offline) + `"use client"` editor `fetch`ing the route → EN/FI/ET `<x>` namespace (key parity test-locked, copy `scripts/site-settings.test.mjs` / `theme.test.mjs` shape).
