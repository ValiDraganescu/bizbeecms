...
- DONE: BUG [P1]: deploy timeline mixed events from DIFFERENT deploy runs into one list — reported 2026-06-18. Fixed with a per-run id. (1) migration `0004_legal_fabian_cortez.sql` (drizzle-generated) adds nullable `deploy_id TEXT` to deploy_events; (2) deployer `startDeploy` mints one `crypto.randomUUID()` per invocation, passes it via env as `DEPLOY_ID` (like SITE_ID) → bash `emit_event` + both `report` bodies include `deployId`; (3) `ParsedDeployEvent`/`parseDeployEvent`/`insertDeployEvent`/`buildFailedCallbackEvent` thread `deployId` (null when absent); (4) new pure `selectLatestRun(events)` (picks the deployId of the max-startedAt event, NaN-safe, legacy null rows = one group) runs BEFORE `collapseDeployEvents` in `deploy-timeline.tsx`. Group-by-run/switcher NOT built (YAGNI). 8 new/updated unit tests → 50 pass. Gate: npm test + opennextjs build + deployer dry-run all green. DRIVER: apply migration 0004 to remote D1 + redeploy both workers. (2026-06-18)
