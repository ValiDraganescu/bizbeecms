...
- **`CMS/cloudflare-env.d.ts` is .gitignored — a fresh clone (the deploy container) has NEITHER the
  Workers global types (`R2ObjectBody`, `R2Bucket`, `D1Database`, `Ai`) NOR the typed `CloudflareEnv`
  env interface.** That file is generated by `wrangler types` (`npm run cf-typegen`) and provides
  BOTH. Local builds pass only because a warm copy exists. So a `Cannot find name 'R2ObjectBody'`
  type error is really "the generated type file is missing", not a missing import. FIX is two-pronged
  and BOTH are needed: (1) `@cloudflare/workers-types` is a CMS devDependency pinned in tsconfig
  `compilerOptions.types` → global Workers types resolve from node_modules without typegen (covers
  editors/tsc/clean installs, can't silently drift); (2) the deployer container build script
  (deployer/src/index.ts) runs `npm run cf-typegen` after `npm ci`, before the opennext build → regen
  `CloudflareEnv` (env shape) from wrangler.jsonc. **If you ever set tsconfig `types`, you MUST list
  `node`+`react`+`react-dom` too** — `types` is a RESTRICTING allowlist; omitting them drops node/react
  ambient globals and breaks the build. **To reproduce the container, you MUST delete cloudflare-env.d.ts**
  (`rm -f CMS/cloudflare-env.d.ts`) before `npm ci`+build — a warm copy hides the bug entirely.
