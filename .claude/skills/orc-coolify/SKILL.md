...
Stop and tell the user to set them in **Settings → Integrations → Coolify**, then restart this terminal so the new env vars are picked up. Don't ask them to paste secrets into the chat.
...
# Step 2 — Create a new app from a private GitHub repo

Endpoint: `POST /api/v1/applications/private-github-app`.

Required body fields:
- `project_uuid` — from Step 1
- `server_uuid` — from Step 1
...
Common optional fields you should set explicitly rather than rely on defaults:
