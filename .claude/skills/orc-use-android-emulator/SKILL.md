---
name: orc-use-android-emulator
version: 1.0.0
description: Build, test, and automate Android apps in the Emulator via accessibility-driven navigation (uiautomator tree, not pixel coordinates). 20 bundled scripts under scripts/ for semantic UI navigation, gradle build automation, accessibility audits, and emulator lifecycle management. Use when driving the Android Emulator, running an Android app, tapping/typing through Android UI, or building/testing a Gradle project. Android counterpart of orc-use-ios-simulator.
---
...
The scripts resolve the Android SDK in this order: `ANDROID_HOME` / `ANDROID_SDK_ROOT`, then the standard macOS location `~/Library/Android/sdk`, then `$PATH`. No env export is required if Android Studio installed the SDK in the default location — `adb` and `emulator` are found automatically.

- **adb + emulator** (platform-tools + emulator packages) — required for everything. Ship with Android Studio.
- **avdmanager / sdkmanager** (the `cmdline-tools` package) — required only by `emulator_create.py` / `emulator_delete.py`. If absent, create AVDs via Android Studio's Device Manager (boot/build/navigate scripts don't need it).
- **A Gradle wrapper** (`./gradlew` in the project root) — required by `build_and_test.py`.
