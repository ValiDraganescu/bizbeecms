...
def _resolve_adb() -> str:
    """Resolve `adb` to an absolute path: PATH, then env-named or standard
    macOS SDK root. Mirrors common.device_utils.resolve_tool — kept local
    because this script intentionally has no `common` import."""
            ...
            for line in result.stdout.split("\n"):
                if "emulator" in line and "device" in line:
                    ...
                    avd_result = subprocess.run(
                        [_resolve_adb(), "-s", serial, "emu", "avd", "name"],
                        capture_output=True,
                        text=True,
                    )
