...
from common.device_utils import get_connected_devices, list_devices, resolve_tool
...
class EmulatorBooter:
    ...
    def boot(
        ...
        cmd = [resolve_tool("emulator"), "-avd", self.avd_name]
        if headless:
    ...
    def _is_boot_completed(self, serial: str) -> bool:
        ...
        """
        try:
            cmd = [resolve_tool("adb"), "-s", serial, "shell", "getprop", "sys.boot_completed"]
    ...
    def _get_avd_name_for_serial(self, serial: str) -> Optional[str]:
        ...
        """
        try:
            cmd = [resolve_tool("adb"), "-s", serial, "emu", "avd", "name"]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=5)
...
def list_avds() -> list:
    ...
    """
    try:
        cmd = [resolve_tool("emulator"), "-list-avds"]
