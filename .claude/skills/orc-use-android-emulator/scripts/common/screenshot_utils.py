#!/usr/bin/env python3
"""
Screenshot utilities for Android devices and emulators with dual-mode support.
...
try:
    from .device_utils import resolve_tool
...
def capture_screenshot(
    ...
    """Capture screenshot from Android device/emulator with flexible output modes.

    Supports both file-based (persistent artifacts) and inline base64 modes
    (for vision-based automation).

    Args:
        serial: Device serial number (uses default device if None)
    ...
    Example:
        # File mode
        result = capture_screenshot('emulator-5554', app_name='MyApp')
        print(f"Saved to: {result['file_path']}")

        # Inline mode
        result = capture_screenshot('emulator-5554', inline=True, size='half')
...
def format_screenshot_result(result: dict) -> str:
    ...
    Example:
        result = capture_screenshot('emulator-5554', inline=True)
