...
# Candidate SDK roots, in priority order: explicit env first, then the
# standard macOS install location. `xcrun`/`simctl` are always on the default
# macOS PATH, so the iOS skill never needs this — `adb`/`emulator` are not, and
# many machines have an installed SDK with nothing exported. Resolving the SDK
# ourselves means the skill works out of the box wherever Android Studio put it.
def _sdk_roots() -> list:
    roots = []
...
# Relative locations of each SDK tool under an SDK root, in priority order.
_TOOL_SUBPATHS = {
...
def resolve_tool(name: str) -> str:
    """
    Resolve an Android SDK executable to an absolute path.

    Order: PATH (honors a user who exported the SDK) → known sub-paths under
    each SDK root (env vars then ~/Library/Android/sdk). Falls back to the bare
    name so the resulting error message still names the missing tool.
    """
    found = shutil.which(name)
    if found:
        return found
    for root in _sdk_roots():
        for sub in _TOOL_SUBPATHS.get(name, [name]):
...
def get_connected_devices() -> list:
    ...
    """
    try:
        cmd = [resolve_tool("adb"), "devices", "-l"]
